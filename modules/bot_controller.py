...
class BotController:
    ...
    def apply(self, config: dict, players: list[str]) -> list[str]:
        """Push new config into the live subsystems.

        Returns the names of subsystems whose enabled flag changed but cannot
        take effect mid-run (they are only created/destroyed at launch), so
        the GUI can tell the user a restart is needed.
        """
        needs_restart = []
        if self.engine:
            ...
            self.engine.set_players(players)
        # DetectionEngine ignores the enabled flag once running, so either
        # flip direction requires a relaunch.
        if config.get("detection", {}).get("enabled", False) != (self.detection is not None):
            needs_restart.append("detection")
        if self.detection:
            self.detection.update_config(config)
        # Audio honors enabled=False live via play_event; only enabling a
        # never-created manager needs a relaunch.
        if config.get("audio", {}).get("enabled", True) and self.audio is None:
            needs_restart.append("audio")
        if self.audio:
